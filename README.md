# Ahsan Rehmat

**Digital Operations & Creative Technology Specialist**

I help founders, agencies, and content teams turn ideas into finished work. My
strongest lanes are video editing, client management, digital-content
operations, and team coordination, supported by practical WordPress and Android
product experience.

## What I bring

- Video editing for documentaries, YouTube, talking-head, gaming, short-form,
  and retention-focused content
- End-to-end client handling: briefs, pricing, scheduling, revisions, issue
  resolution, and delivery
- Content operations and quality review for multi-channel production
- Responsive WordPress websites, landing pages, forms, and launch support
- Kotlin and Jetpack Compose prototyping for practical Android products

## Selected proof

| | |
|---|---|
| **30+ international clients** | Freelance delivery across the US, UK, Germany, the Netherlands, and Nigeria |
| **4.9 Fiverr rating** | Across 24 completed orders |
| **Teams of up to 10** | Editors, scriptwriters, and graphic designers |
| **Freelancing since 2021** | Creative production, client operations, and digital delivery |

## Featured project

### PulsePlan

An Android workout-planning MVP built with Kotlin and Jetpack Compose. It
creates clear daily and weekly plans from a user's goals, experience, equipment,
schedule, workout style, and movement preferences.

The current build includes personalized onboarding, focused workout sessions,
progress history, reminders, recovery-aware streaks, exercise guides, and
same-equipment alternatives. A safe local preview is available while live
Firebase setup remains pending.

**Built with:** Kotlin · Jetpack Compose · Material 3 · DataStore · WorkManager

## Other work

- **Accounting business website:** built a responsive WordPress site for a
  Dubai accounting and tax-services company, including setup, pages, forms,
  visual customization, and launch preparation.
- **Multi-channel content operations:** co-managed four digital news channels
  and coordinated a 10-person remote team producing up to eight videos per day
  across the portfolio. Client identities remain confidential.
- **Cars with a Story:** developed the content positioning and production
  workflow for an independent automotive storytelling channel.

## Let's work together

I am based in Mianwali, Pakistan and available for remote freelance work.

- Email: [ahsanrehmat193@gmail.com](mailto:ahsanrehmat193@gmail.com)
- WhatsApp: [Start a project conversation](https://wa.me/923254172193)
